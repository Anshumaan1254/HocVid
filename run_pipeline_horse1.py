...
print("Downloading HF weights for AFLB...")
...
print("Running Full Sequential Cascade Pipeline...")
with torch.no_grad():
    # model() internally executes AFLB -> DDER -> Degradation Map
    degradation_map = model(img_padded)

# Normalize the residual degradation map out to [0,1] for viewing
deg_min = degradation_map.min()
deg_max = degradation_map.max()
degradation_vis = (degradation_map - deg_min) / (deg_max - deg_min + 1e-8)

output_crop = degradation_vis[:, :, :h_orig, :w_orig].clamp(0, 1)
output_pil = TF.to_pil_image(output_crop.squeeze(0).cpu())

if output_pil.size != (orig_w, orig_h):
    output_pil = output_pil.resize((orig_w, orig_h), Image.LANCZOS)

output_path = os.path.join(os.path.dirname(__file__), 'stage2bacha.png')
...
if os.path.exists(artifact_dir):
    shutil.copy(output_path, os.path.join(artifact_dir, 'stage2bacha.png'))
