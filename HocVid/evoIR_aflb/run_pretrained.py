...
class FrequencyGuidedAttentionModule(nn.Module):
    def __init__(self, in_channels, out_channels, decoder_flag=False):
        ...
        # REVERT Bug 5 fix: HF pretrained weights were trained with the bug (num_heads=48).
        # We MUST use out_channels as num_heads here to load them.
        self.high_freq_attention=FrequencyGuidedAttention(in_channels,num_heads=out_channels)
        self.low_freq_attention=FrequencyGuidedAttention(in_channels,num_heads=out_channels)
        if decoder_flag:
...
if matched > 0:
    print(f"\n[OK] Successfully loaded {matched} pretrained parameter tensors!")
else:
    print("\n[WARNING] No keys matched. Running with partial/random weights.")
...
input_path = r'D:\IEEE 1\image.png'
...
print(f"\n{'='*60}")
print("[OK] DONE! Pretrained EvoIR AFLB inference complete!")
print(f"{'='*60}")
