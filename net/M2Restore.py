"""
M2Restore.py -- Fixed M2Restore for HoCVid (All 9 Bugs Resolved)
================================================================

Standalone M2Restore model for training/testing on AllWeather dataset.
This is SEPARATE from m2restore_decoder.py (HoCVid's decoder head).

Bug fixes applied:
  2A: Mamba import fallback (pure PyTorch SSM if mamba_ssm unavailable)
  2B: DA-CLIP import path fixed for HoCVid project structure
  2C: DA-CLIP checkpoint path auto-search
  2D: Text tokens use register_buffer (no device hardcoding)
  2E: deprecated sigmoid API -> torch.sigmoid()
  2F: torch.abs() on IFFT -> .real (fixes color tinting)
  2G: Frequency mask zero for small grids -> fractional sizing
  2H: Dict in nn.Sequential -> tuple-based passing
  2I: deprecated autocast API -> torch.amp.autocast
"""
...
try:
    ...
    Mamba = None
_mamba_backend = "mamba_ssm (CUDA)" if _MAMBA_AVAILABLE else "PyTorch SSM fallback"
print(f"[M2Restore] SSM backend: {_mamba_backend}")
...
# == Bug 2I: Updated autocast import ===================================
from torch.amp import autocast

# == Bug 2B: DA-CLIP import path ========================================
_m2_dir = os.path.dirname(os.path.abspath(__file__))
_daclip_dir = os.path.normpath(
    os.path.join(_m2_dir, '..', 'DDER', 'daclip')
)
if _daclip_dir not in sys.path:
    sys.path.insert(0, _daclip_dir)

# Also add DDER dir for MOE_CLIPbias
_dder_dir = os.path.normpath(os.path.join(_m2_dir, '..', 'DDER'))
...
class MB(nn.Module):
    ...
    # == Bug 2H: Tuple-based passing instead of dict ================
    def forward(self, x):
        # x is a tuple (feature_tensor, prompt_tensor)
        x, prompt = x
        residual = x
        x = self.norm1(x)
...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, prompt_dim=512):
        ...
        if _MAMBA_AVAILABLE:
            ...
            _inner = dim * expand
            self.fb_in_proj  = nn.Linear(dim, _inner * 2)
            self.fb_conv1d   = nn.Conv1d(
                _inner, _inner, kernel_size=d_conv,
                padding=d_conv - 1, groups=_inner
            )
            self.fb_act      = nn.SiLU()
            self.fb_A_log    = nn.Parameter(
                torch.log(
                    torch.arange(1, d_state + 1, dtype=torch.float32).repeat(_inner, 1)
                )
            )
            self.fb_D        = nn.Parameter(torch.ones(_inner))
            self.fb_dt_proj  = nn.Linear(_inner, _inner)
            self.fb_out_proj = nn.Linear(_inner, dim)
    ...
    def _fallback_ssm(self, x):
        """
        Pure PyTorch SSM. Implements discretized selective state space.
        x: (B, L, dim) -> (B, L, dim)
        """
        B, L, _ = x.shape
        xz = self.fb_in_proj(x)
        x_in, z = xz.chunk(2, dim=-1)
        x_in = self.fb_conv1d(
            x_in.transpose(1, 2)
        )[..., :L].transpose(1, 2)
        x_in = self.fb_act(x_in)
        A  = -torch.exp(self.fb_A_log.float())
        dt = F.softplus(self.fb_dt_proj(x_in))
        dA = torch.exp(dt.unsqueeze(-1) * A)
        y  = (x_in.unsqueeze(-1) * dA).sum(-1)
        y  = y + x_in * self.fb_D
        y  = y * self.fb_act(z)
        return self.fb_out_proj(y)

    # == Bug 2I: Updated autocast decorator =========================
    ...
    def forward(self, x, prompt):
        ...
        # == Bug 2A: Mamba or fallback ==============================
        if self.use_mamba:
            x1_mamba = self.mamba(x1_norm)
        else:
            x1_mamba = self._fallback_ssm(x1_norm)
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        # -- Bug 2E: use torch.sigmoid (non-deprecated) --
...
class FreModule(nn.Module):
    ...
    def fft(self, x, n=128):
        ...
        # == Bug 2G: Fractional mask sizing =========================
        for i in range(mask.shape[0]):
            t_h = float(threshold[i, 0, :, :].mean())
            t_w = float(threshold[i, 1, :, :].mean())
            h_ = max(1, int(h * t_h * 0.5))
            w_ = max(1, int(w * t_w * 0.5))
            # Clamp: must be at least 1px, at most half the dim minus 1
            h_ = min(h_, max(1, h // 2 - 1))
            w_ = min(w_, max(1, w // 2 - 1))
        ...
        # == Bug 2F: .real instead of torch.abs() ==================
        high = high.real
...
class M2Restore(nn.Module):
    def __init__(self, 
        ...
        # == Bug 2C: DA-CLIP checkpoint path auto-search ============
        _net_dir = os.path.dirname(os.path.abspath(__file__))
        _candidates = [
            os.path.join(_net_dir, '..', 'DDER', 'daclip', 'daclip_ViT-B-32.pt'),
            os.path.join(_net_dir, '..', 'DDER', 'daclip_ViT-B-32.pt'),
            os.path.join(_net_dir, '..', 'DDER', 'pretrained', 'daclip_ViT-B-32.pt'),
            os.path.join(_net_dir, 'daclip', 'pretrained', 'daclip_ViT-B-32.pt'),
        ]
        checkpoint = None
        ...
        if checkpoint is None:
            raise FileNotFoundError(
                "[M2Restore] daclip_ViT-B-32.pt not found. Searched:\n" +
                "\n".join(f"  {os.path.normpath(c)}" for c in _candidates) +
                "\nSolution: place weights at DDER/daclip/daclip_ViT-B-32.pt"
            )
        print(f"[M2Restore] DA-CLIP checkpoint: {checkpoint}")
        ...
        # == Bug 2D: register_buffer (device-agnostic) ==
        self.register_buffer('text_tokens', tokenizer(degradations))
    ...
    def forward(self, inp_img, noise_emb=None):
        # == Bug 2I: Updated autocast ==============================
        with torch.no_grad(), torch.amp.autocast('cuda'):
            crop_img = self.preprocess(inp_img)
            # == Bug 2D: use registered buffer =====================
            text_features = self.daclip.encode_text(self.text_tokens)
        ...
        latent, _ = self.latent(
            (inp_enc_level4, image_features.float())
        )
